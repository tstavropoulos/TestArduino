...
%  Task runs in batches of specified duration, with InterBatchIntervals, also
%    specified.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function ImageBatchTask
clear all;
%#ok<*MSNU>
global bQuit;
global cFlag;
fprintf('Image Batch Task.\n');
...
%Batch Values
BatchDuration = 10.0 * 60.0; %10 Minutes (in seconds)
IBIDuration = 15.0 * 60.0; %15 Minutes (in seconds)
BatchNumber = 10;
...
try
	...
	batchProgressTime = toc + initialDelay + BatchDuration;
    while 1
        ...
        if (currentState==STATE_ITI)
			...
			if ( toc > batchProgressTime )
				submitDataFlag('EndBatch');
				BatchNumber = BatchNumber - 1;
				if (BatchNumber <= 0)
					currentState = STATE_WAIT_FOR_END;
					progressTime = progressTime + FinalDelay;
				else
					currentState = STATE_IBI;
					batchProgressTime = batchProgressTime + IBIDuration;
				end
            elseif ( toc > progressTime )
		...
		elseif (currentState==STATE_IBI)
			if (toc > batchProgressTime )
				submitDataFlag('BeginBatch');
				currentState = STATE_ITI;
				progressTime = toc + ITIDuration;
				batchProgressTime = batchProgressTime + BatchDuration;
			end
        end
